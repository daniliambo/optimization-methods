...
def plot_trajectory(func, history, fit_axis=False, label=None, c=None):
    """
    ...
    if c is None:
        ...
        c = ['red', 'blue', 'green', 'orange'][c]

    x_values, y_values = zip(*history)
    plt.plot(x_values, y_values, '-o', linewidth=2.0, ms=8.0,
             alpha=0.7, c=c, label=label)
